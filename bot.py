...
def is_greeting_message(message_text):
    """মেসেজটি গ্রিটিংস কিনা চেক করবে - ফিক্সড ভার্সন"""
    if not message_text:
        return False
        
    message_lower = message_text.lower().strip()
    
    # শব্দ ভিত্তিতে ম্যাচ চেক
    message_words = set(message_lower.split())
    
    for greeting in GREETINGS_KEYWORDS:
        greeting_lower = greeting.lower()
        # সম্পূর্ণ ম্যাচ বা শব্দ ভিত্তিতে ম্যাচ
        if (greeting_lower in message_lower or 
            greeting_lower in message_words or
            any(word in greeting_lower for word in message_words)):
            print(f"✅ গ্রিটিংস ম্যাচ পেয়েছি: '{greeting}' -> '{message_text}'")
            return True
    
    print(f"❌ গ্রিটিংস ম্যাচ নেই: '{message_text}'")
    return False
...
async def handle_auto_search(update: Update, query: str):
    ...
    try:
        ...
        if match_score >= 85:  # এক্সাক্ট ম্যাচ
            # ✅ যদি ১টির বেশি মুভি থাকে
            if len(results) > 1:
                await update.message.reply_text(
                    f"🎬 **'{query}' - পাওয়া ভার্সনগুলো ({len(results)} টি):**",
                    parse_mode='Markdown'
                )
                ...
                if len(results) > 3:
                    await update.message.reply_text(
                        f"📦 _এবং আরও {len(results) - 3} টি মুভি..._",
                        parse_mode='Markdown'
                    )
            else:
                # শুধু ১টি থাকলে
        ...
        # ✅ পরিবর্তিত error মেসেজ
        error_message = f"""
⚠️ **'{query}' নামে সার্চ করতে সমস্যা হচ্ছে**
...
📝 **উদাহরণ:**
`kgf` (❌ `kgf 2 full movie hindi`)
`rrr` (❌ `rrr movie 2022`)
`dhoom` (❌ `dhoom three`)

🔄 **আবার চেষ্টা করুন - নিচের মতো করে লিখুন:**
`{query.split()[0]}` 🔥
"""
        await update.message.reply_text(
            error_message, 
...
async def button_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if callback_data.startswith("confirm_"):
        ...
        if results:
            ...
            await query.edit_message_text(
                f"✅ {query.from_user.first_name}, আপনার মুভিটি পাওয়া গেছে!",
        ...
        guidance_message = f"""
🔍 '{original_query}' আপনার পছন্দের মুভি না?

🎯 **সঠিকভাবে সার্চ করার টিপস:**
• শুধু মুভির মূল নাম লিখুন
• ইংলিশে লিখুন (বাংলা থেকে অটো ট্রান্সলেশন হবে)
• স্পেসিং এবং বানান চেক করুন

📝 **উদাহরণ:**
`kgf` (❌ `kgf 2 full movie`)
`avatar` (❌ `avatar the way of water`)
`bahubali` (❌ `বাহুবলী`)

🔄 **আবার চেষ্টা করুন - সঠিক নাম লিখুন:**
`{original_query.split()[0] if original_query.split() else original_query}`
"""
        ...
        if results and len(results) > 1:
            ...
            await query.edit_message_text(
                f"🤔 আপনি কি '{alternative_movie['title']}' খুঁজছেন?",
            ...
            await query.edit_message_text("😔 আর কোনো সাজেশন নেই। নতুন করে সার্চ করুন।")
    
    elif callback_data == "show_search_guide":
        search_guide = """
🎬 **সঠিকভাবে মুভি সার্চ করার গাইড**
...
❌ **ভুল উপায়:**
`kgf 2 full movie hindi` → `kgf`
`avatar the way of water` → `avatar`  
`বাহুবলী` → `bahubali`

💡 **টিপস:**
• বট বাংলা নাম অটো ট্রান্সলেট করতে পারে
• স্পেলিং ভুল হলে বট করেক্ট করবে
• সমস্যা হলে এডমিনকে জানান

🔄 **এখনই ট্রাই করুন - নিচের মতো লিখুন:**
`/search মুভির_নাম`
"""
